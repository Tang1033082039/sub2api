<template>
  <div class="flex flex-wrap items-center gap-3">
    ...
    <Select :model-value="filters.cleanup_status" class="w-40" :options="cleanupOpts" @update:model-value="updateCleanupStatus" @change="$emit('change')" />
...
const updateCleanupStatus = (value: string | number | boolean | null) => { emit('update:filters', { ...props.filters, cleanup_status: value }) }
...
const cleanupOpts = computed(() => [{ value: '', label: t('admin.accounts.allCleanupStatus') }, { value: 'pending', label: t('admin.accounts.cleanupPending') }])
